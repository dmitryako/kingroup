 ...
 */
public class RkLogCRJUnit extends TestCase {
  public static Test suite() {
    return new TestSuite(RkLogCRJUnit.class);
  }
...
//      BooleQuadr w = new BooleQuadr(x);
    ...
    FuncVec Y = new YkLogCR(wCR.getLogCRToR(), f, f, 0).calcYk();
    assertEquals(0, Math.abs(T.distSLOW(Y)), 2e-9);
    res = RkLCR.calcRkLogCR(wCR, f, f, f, f, 0);
    ...
    res = RkLCR.calcRkLogCR(wCR, f, f2s, f, f2s, 0);
    ...
    res = RkLCR.calcRkLogCR(wCR, f2p, f, f2p, f, 0);
    ...
    res = RkLCR.calcRkLogCR(wCR, f2s, f2p, f2s, f2p, 0);
    ...
    res = RkLCR.calcRkLogCR(wCR, f2p, f2p, f2p, f2p, 0);
    ...
    res = RkLCR.calcRkLogCR(wCR, f2p, f2p, f2p, f2p, 1);
    assertEquals(0, Math.abs(res - 0.12044270784428178), 2e-20); // at x=-4, 1/16, 220
    res = RkLCR.calcRkLogCR(wCR, f2p, f2p, f2p, f2p, 2);
