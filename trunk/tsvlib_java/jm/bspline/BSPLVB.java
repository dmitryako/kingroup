 ...
 * User: jc138691, Date: 14/04/2005, Time: 10:14:47
 */
public class BSPLVB {
//   parameter (jmax = 20)
  private static int jmax = 20;
//      save j,deltal,deltar
//      data j/1/
  private static int j = 0;
//      real biatx(jhigh),t(1),x,   deltal(jmax),deltar(jmax),saved,term
  private static double[] deltal = new double[jmax];
  private static double[] deltar = new double[jmax];
...
//      go to (10,20), index
    if (index == 1) {
      j = 1; //      10 j = 1
      biatx[1 - 1] = 1; //      biatx(1) = 1.
      if (j >= jhigh) //      if (j .ge. jhigh)                 go to 99
        return;
    }
    do {
      int jp1 = j + 1;//      20    jp1 = j + 1
      deltar[j - 1] = t[left + j - 1] - x; //      deltar(j) = t(left+j) - x
      deltal[j - 1] = x - t[left + 1 - j - 1];//      deltal(j) = x - t(left+1-j)
      double saved = 0.; //saved = 0.
      for (int i = 1; i <= j; i++) {//      do 26 i=1,j
        double term = biatx[i - 1] / (deltar[i - 1] + deltal[jp1 - i - 1]);//      term = biatx(i)/(deltar(i) + deltal(jp1-i))
        biatx[i - 1] = saved + deltar[i - 1] * term;//      biatx(i) = saved + deltar(i)*term
        saved = deltal[jp1 - i - 1] * term;//      26       saved = deltal(jp1-i)*term
      }
      biatx[jp1 - 1] = saved;//      biatx(jp1) = saved
      j = jp1; //      j = jp1
    } while (j < jhigh); //      if (j .lt. jhigh)              go to 20
//      99                                   return
  }
}
/*
